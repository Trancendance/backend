...
class Game {
	...
	updateState = (): ServerResponse => {
		...
		return {
			...
			ball: { x: this._gameState.ball.position.x, y: this._gameState.ball.position.y },
		};
	};
	...
	listenPlayerMoves = (playerId: number, newY: number) => {
		...
		if (player) {
			...
			if (player.position.y + this._staticDefs.playerSize.y > this._staticDefs.height)
				player.position.y = this._staticDefs.height - this._staticDefs.playerSize.y;
		}
...
const start = async () => {
	try {
	  ...
	  console.log("🚀 WebSocket fake game server en:");
